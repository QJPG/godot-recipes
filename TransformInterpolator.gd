...
func SetReset(itemNode: Node3D) -> void:
	...
	reset_transform = from.global_transform
...
func Reset(interpolateTime := 0.5) -> void:
	...
	var x = from.global_transform.interpolate_with(reset_transform, reset_delay)
	var t = from.create_tween() as Tween
	t.tween_property(from, "global_transform", x, interpolateTime)
...
func _process(delta):
	...
	var y = from.global_transform.interpolate_with(x, look_at_delay * delta)
